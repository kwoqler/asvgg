...
variavel_sem_atribuicao: TIPO ID TERMINO?;
...
expressao: exp_atribuicao | exp_imprima | exp_aritmetica | exp_logica | ID | NUMEROS;
...
exp_logica: (NUMEROS|ID) OPL (NUMEROS|ID) TERMINO?;
op_arit: ADC | SUB | MULT | DIV | ATRIBUICAO;


//declaração de função
decl_funcao: TIPO ID '(' (variavel','?)*')''{' (expressao|instrucao_de_controle)+ '}';

//declaração programa principal
decl_main: MAIN'()''{' (expressao|instrucao_de_controle)+ '}';